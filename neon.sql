...
create table if not exists private.orders (
  ...
  driver_user_id uuid not null references private.app_users(id) on delete restrict,
...
begin
  ...
  if v_actor.role in ('admin', 'sales') then
    ...
    join private.app_users d on d.id = o.driver_user_id
...
begin
  ...
  if v_actor.role = 'driver' and v_order.driver_user_id <> v_actor.id then
